...
// Phương thức mới sử dụng JPQL để xử lý tham số isRead là NULL
	...
	@Modifying
	@Query("UPDATE Notification n SET n.read = :isRead, n.readAt = CASE WHEN :isRead = true THEN CURRENT_TIMESTAMP ELSE NULL END " +
	...
	@Query("SELECT COUNT(n) FROM Notification n WHERE n.receiver.id = :receiverId AND n.read = :isRead")
	Long getCountNotificationByIsRead(@Param("isRead") boolean isRead, @Param("receiverId") String receiverId);

	@Modifying
	@Query("UPDATE Notification n SET n.read = :isRead WHERE n.id IN :ids")
