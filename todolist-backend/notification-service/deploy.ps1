# ======================================================
# 🚀 AUTO DEPLOY SCRIPT: notification-service (AWS ECS)
# ======================================================
...
# ====== 2. THÔNG TIN DỊCH VỤ (NOTIFICATION) ======
# 👇 ĐÃ CẬP NHẬT TẠI ĐÂY
$SERVICE_NAME = "notification-service"
$TASK_DEFINITION_NAME = "notification-service-td"
$LOG_GROUP_NAME = "/ecs/notification-service-td"

# ====== 3. CẤU HÌNH ECR ======
# Lưu ý: Đảm bảo bạn đã tạo Repository tên 'notification-service' trên ECR
...
# 3.3 Kiểm tra Service Connect / Service Discovery (Vì Notification không dùng Load Balancer)
Write-Host "`n🎯 Kiểm tra Service Discovery:"
# Với Notification Service, chúng ta không dùng Load Balancer nên script sẽ báo "không gắn LB".
# Điều này là HOÀN TOÀN ĐÚNG với kiến trúc mới.
try {
    ...
    if ($serviceDetails.services[0].loadBalancers.Count -gt 0) {
        ...
        if ($tgHealth.TargetHealthDescriptions.Count -gt 0) {
            $tgHealth.TargetHealthDescriptions | ForEach-Object { 
                ...
                Write-Host "   Target: $($_.Target.Id) | Trạng thái: $state" -ForegroundColor $color
            }
        }
    } else {
        Write-Host "ℹ️ Service này chạy Internal (Service Discovery), không cần Load Balancer Health Check." -ForegroundColor Green
        Write-Host "   -> Vui lòng kiểm tra logs ở trên để đảm bảo ứng dụng khởi động thành công."
    }
} catch {
    Write-Host "❌ Lỗi khi kiểm tra thông tin Service." -ForegroundColor Red
}
