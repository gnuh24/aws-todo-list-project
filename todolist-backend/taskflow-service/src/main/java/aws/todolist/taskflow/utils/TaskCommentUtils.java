...
public class TaskCommentUtils {
    ...
    public TaskComment getCommentAndCheck(String idComment, Account account) {
        ...
        if (taskComment == null) {
            ...
            throw new ResourceNotFoundException(BusinessErrorCode.TASKFLOW_NOT_FOUND, "Comment không tồn tại hoặc đã bị xóa");
        }
        ...
        if (!taskComment.getAccount().getId().equals(account.getId())) {
            ...
            throw new ForbiddenException(BusinessErrorCode.TASKFLOW_ACCESS_DENIED, "Bạn không thể chỉnh sửa comment của người khác");
        }
