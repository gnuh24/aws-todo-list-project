...
public class NotificationUtils {
    ...
    public Set<Account> getReceiversForTask(Task task) {
        ...
        String accountAuthorId = getAccountAuthorId();

        if (task.getAccountAssign() != null && !Objects.equals(task.getAccountAssign().getId(), accountAuthorId)) {
            receivers.add(task.getAccountAssign());
        }

        if (!Objects.equals(task.getCreatedByAccount().getId(), accountAuthorId)) {
    ...
    public Set<Account> getReceiversForTaskComment(Task task) {
        ...
        for (TaskComment comment : task.getTaskComments()) {
            if (!Objects.equals(comment.getAccount().getId(), accountAuthorId)) {
    ...
    private String getAccountAuthorId() {
    ...
    private Account getAccountAuthor() {
        String accountId = getAccountAuthorId();
            ...
            try {
                ...
                switch (message.getType()) {
                    ...
                    case PROJECT_MEMBER_ADDED: {
                        ...
                        message.setContent(String.format(
                                "'%s' đã được thêm vào dự án '%s' với vai trò '%s'.",
