# ======================================================
# 🚀 AUTO DEPLOY SCRIPT: taskflow-service (AWS ECS/Fargate)
# ======================================================
...
# ====== 2. THÔNG TIN DỊCH VỤ (TASKFLOW SERVICE) ======
$SERVICE_NAME = "taskflow-service"
$TASK_DEFINITION_NAME = "taskflow-service-td"
$LOG_GROUP_NAME = "/ecs/taskflow-service-td"

# ====== 3. CẤU HÌNH ECR ======
# Lưu ý: Đảm bảo bạn đã tạo Repository tên 'taskflow-service' trên ECR
    ...
    if ($serviceDetails.services[0].loadBalancers.Count -gt 0) {
        ...
        if ($tgHealth.TargetHealthDescriptions.Count -gt 0) {
            $tgHealth.TargetHealthDescriptions | ForEach-Object { 
                ...
                Write-Host "   Target: $($_.Target.Id) | IP: $($_.Target.Id) | Trạng thái: $state" -ForegroundColor $color
